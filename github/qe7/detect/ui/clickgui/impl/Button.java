...
import github.qe7.detect.ui.clickgui.impl.setting.SettingBoolean_;
import github.qe7.detect.ui.clickgui.impl.setting.SettingComponent;
import github.qe7.detect.ui.clickgui.impl.setting.SettingMode_;
import github.qe7.detect.ui.clickgui.impl.setting.SettingNumber_;
...
public class Button {
    ...
    public double drawScreen(int mouseX, int mouseY, float partialTicks, double plusplus) {
        ...
        Gui.drawRect(panel.x,panel.y + y + plusplus, panel.x + panel.width, panel.y + y + height + plusplus, module.isToggled() ? new Color(47, 47, 47, 255).getRGB() : new Color(33, 33, 33, 255).getRGB());
        font.drawStringWithShadow(module.getName(), panel.x + 4, panel.y + y + ((height - mc.tenacity.getHeight()) / 2) - 1 + plusplus, -1);
        if (module.getKey() != 0)
            font.drawStringWithShadow(binding ? "[binding...]" : "[" + Keyboard.getKeyName(module.getKey()) + "]", panel.x + panel.width - font.getStringWidth(binding ? "[binding...]" : "[" + Keyboard.getKeyName(module.getKey()) + "]") - 4, panel.y + y + ((height - mc.tenacity.getHeight()) / 2) - 1 + plusplus, -1);
        return setHeight;
    }
