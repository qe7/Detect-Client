...
public class Hud extends Module {
    ...
    public void onEvent(Event event) {
        if (event instanceof EventRender2D) {
            ...
            Collections.sort(modules, new sort());

            font.drawStringWithShadow("§4D§7etect", 4, 4, -1);

            int count = 0;
            for (Module module : modules) {
                if (module.isToggled()) {
                    String text = module.getName() + (module.getSuffix().length() > 0? " \2477" + module.getSuffix() + "" : "");
                    ...
                    double endX = ((EventRender2D) e).getWidth() - 4;
